...
DEST_HOST  = "carmack.stanford.edu"
DEST_USER  = "bettingr"
DEST_GROUP = "scg_seq"
...
if not opts.rsync:
    ...
    retcode = subprocess.call(["ssh", "-S", SSH_CM_SOCKET, "-l", opts.user, opts.host,
                               "sg %s 'tar -C %s -x'" % (opts.group, opts.dest_root)],
    ...
    #
    rsync_cmd_list = ["rsync", "-rlptR", "-e", "ssh -S %s -l %s" % (SSH_CM_SOCKET, opts.user),
...
#
retcode = subprocess.call(["ssh", "-S", SSH_CM_SOCKET, "-l", opts.user, opts.host,
                           "sg %s 'touch %s'" % (opts.group,
                                                 os.path.join(dest_run_path, opts.status_file))])
if retcode:
