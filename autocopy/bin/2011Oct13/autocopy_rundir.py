...
COPY_DEST_HOST  = "carmack.stanford.edu"
...
EMAIL_TO          = 'bettingr@stanford.edu'
...
# Archiving subdirectory, to be created/used within each run root.
SUBDIR_ARCHIVE = "Archived"
# Aborted subdirectory, to be created/used within each run root.
...
def check_run_roots_freespace():
    ...
    if len(too_full_dict.keys()) > 0:
        ...
        for (run_root, stats) in too_full_dict.iteritems():
            email_body += "%s\t%0.1f TiB\n" % (run_root, stats.f_bfree * stats.f_frsize / ONETERA)
...
def phase1_scan_run_roots():
    ...
    for run_root in run_root_list:
        ...
        for entry in curr_dirs:
            ...
            # HACK: Ignore subdirectory "Archived" and "Aborted", where completed directories are moved.
    ...
    for rundir in copying_rundirs:
        ...
        if rundir.copy_proc:
            ...
            if retcode == 0:
                ...
                if run_fields and run_fields["sequencer_done"] == "true":
                    rundir.status = RunDir.STATUS_COPY_COMPLETE
                    rundir.drop_status_file()
                    rundir.copy_proc = None
                    rundir.copy_end_time = datetime.datetime.now()
        ...
        if lims_fields:
            ...
            if not check_lims_msg:
                if lims_status != STATUS_LIMS_OK:
                    ...
                    # If this RunDir has already been copied, but didn't have a LIMS entry
                    #  until now, then we probably need to set the Sequencer Done flag.
                    if rundir.get_status() == RunDir.STATUS_COPY_COMPLETE:

                        #if rundir.get_validated():

                            # Probably couldn't set this earlier if status of LIMS was previously not OK.
                            seq_done_dict = {'sequencer_done': 'yes'}
                            if lims_obj.lims_run_modify_params(rundir.get_dir(), seq_done_dict):
                                log("LIMS: Set Sequencer Done flag of %s to True" % rundir.get_dir())
                            else:
