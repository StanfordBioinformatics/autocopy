...
def validate(rundir, no_cif=False, verbose=False):
    ...
    for lane in lane_list:
        ...
        if (rundir.get_platform() == rundir.PLATFORM_ILLUMINA_GA or
            True): # (rundir.get_platform() == rundir.PLATFORM_ILLUMINA_HISEQ and rundir.get_control_software_version(integer=True) <= 1137)): # "1.1.37"
...
def make_archive_tar(rundir, **opts):
    ...
    if not already_have_interop_tar:
        ...
        if ssh_socket is None:
            ...
            interop_ssh_cmd_list = ["ssh", "-S", ssh_socket, "", "cat - > %s" % (interop_tar_path_tmp)]
